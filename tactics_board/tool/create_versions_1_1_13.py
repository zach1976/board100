#!/usr/bin/env python3
"""Create v1.1.13 App Store Versions + attach 1.1.13 builds for all 16 apps.

Run AFTER all 16 1.1.13 builds reach VALID processing state in ASC.
After this, the apps will be in PREPARE_FOR_SUBMISSION state and
fastlane deliver can patch metadata onto them.

Idempotent: if a v1.1.13 PREPARE_FOR_SUBMISSION version already exists
...
def main():
    ...
    for sku, bundle_id in APPS:
        ...
        if r.get("data"):
            ...
            if existing_v == "1.1.13":
                print(f"  ↷ v1.1.13 draft already exists ({version_id})")
            else:
                print(f"  ⚠ existing draft is v{existing_v}, not v1.1.13 — skipping (resolve manually)")
                err += 1
                continue
        else:
            # 3) Create v1.1.13 App Store Version
            r = api("POST", "/v1/appStoreVersions", {
                "data": {"type": "appStoreVersions",
                         "attributes": {"platform": "IOS", "versionString": "1.1.13"},
                         "relationships": {"app": {"data": {"type": "apps", "id": app_id}}}},
            })
            if "_error" in r:
                errs = r["_error"].get("errors", [{}])
                detail = errs[0].get("detail", str(r["_error"]))[:200]
                print(f"  ✗ create v1.1.13: {detail}"); err += 1; continue
            version_id = r["data"]["id"]
            print(f"  ✓ created v1.1.13 draft ({version_id})")

        # 4) Find the 1.1.13 build
        r = api("GET", f"/v1/builds?filter[app]={app_id}&filter[preReleaseVersion.version]=1.1.13&sort=-uploadedDate&limit=1")
        if not r.get("data"):
            print(f"  ✗ no 1.1.13 build found in ASC"); err += 1; continue
        build = r["data"][0]
        build_id = build["id"]
