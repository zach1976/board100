...
#        python3 tool/check_app_status.py
...
STATE_FILE="build/release_1_1_25_state.txt"
mkdir -p build
touch "$STATE_FILE"
...
if ! did "builds_processed"; then
  ...
  VERSION=1.1.25 python3 tool/wait_builds_processed.py
...
if ! did "ios_submitted_$SUBMIT_SPORT"; then
  ...
  python3 tool/submit_1_1_25.py "$SUBMIT_SPORT"
