#!/bin/bash
# release_1_1_13.sh — end-to-end v1.1.13 feature release (UX overhaul).
#
...
# (screenshots, app previews) carry forward from v1.1.12.
#
...
echo "═════════════════════════════════════════════════════════"
echo "  v1.1.13 UX overhaul release — $(date)"
echo "═════════════════════════════════════════════════════════"
echo ""
echo "Pre-flight:"
...
if ! did "builds_processed"; then
  echo ""
  echo "▶ Phase 3: polling ASC until all 16 v1.1.13 builds reach VALID (~20-30 min)"
  VERSION=1.1.13 python3 tool/wait_builds_processed.py
...
# ───────── Phase 3.5: create v1.1.13 App Store Versions + attach builds ─────────
if ! did "versions_created"; then
  echo ""
  echo "▶ Phase 3.5: create v1.1.13 App Store Versions + attach builds (idempotent)"
  python3 tool/create_versions_1_1_13.py
...
# ───────── Phase 4: upload metadata (screenshots/previews reused from v1.1.12) ─────────
if ! did "metadata_uploaded"; then
  echo ""
  echo "▶ Phase 4: upload version-level metadata (description / subtitle / keywords / release_notes)"
  echo "  (screenshots + app previews on ASC carry forward from v1.1.12 automatically;"
...
if ! did "submitted_for_review"; then
  echo ""
  echo "▶ Phase 5: submit_all.py — submit v1.1.13 for review"
  VERSION=1.1.13 python3 tool/submit_all.py
...
echo ""
echo "═════════════════════════════════════════════════════════"
echo "  🎉 v1.1.13 release submitted for all 16 apps."
