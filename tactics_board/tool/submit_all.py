...
for bundle_id, name in APPS:
    ...
    if not r_ver["data"]:
        # Create a new 1.1.8 version
        r_new = api("POST", "/v1/appStoreVersions", {
            "data": {"type": "appStoreVersions",
                     "attributes": {"platform": "IOS", "versionString": "1.1.8"},
        ...
        # Find the uploaded 1.1.8 build
        r_builds = api("GET", f"/v1/builds?filter[app]={app_id}&filter[preReleaseVersion.version]=1.1.8&sort=-uploadedDate&limit=1")
        if not r_builds.get("data"):
            print(f"  ❌ no 1.1.8 build found in App Store Connect yet, try again later")
            continue
