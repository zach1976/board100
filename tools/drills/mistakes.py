...
# The eight match moments the library had no drill for (see match_moments).
add("soccer", 'Holding the offside line',
    "One defender a step late — he plays the whole line onside and nobody else's timing mattered.",
    '有一个人慢了半步——整条线的越位就全废了，其他人跑得再准也没用。',
    '有一個人慢了半步——整條線的越位就全廢了，其他人跑得再準也沒用。',
    '一人が半歩遅れる——それだけで全員がオンサイドになり、他の誰のタイミングも意味を失う。',
    '한 명이 반 박자 늦으면 라인 전체가 온사이드가 된다 — 나머지의 타이밍은 소용없다.',
    'Un defensa medio paso tarde: habilita a toda la línea y el resto de los tiempos ya no importan.',
    "Un défenseur en retard d'un demi-pas remet toute la ligne en jeu ; le timing des autres ne compte plus.",
    'Satu bek telat setengah langkah — seluruh garis jadi onside dan timing yang lain tak berarti.',
    'Seorang bek lewat setengah langkah membuat seluruh barisan onside.',
    'มีคนช้าไปครึ่งก้าว ทั้งแนวก็ไม่ล้ำหน้า จังหวะของคนอื่นเป็นอันสูญเปล่า',
    'Một hậu vệ chậm nửa nhịp làm cả hàng ngang hết việt vị; thời điểm của những người khác thành vô nghĩa.')

add("soccer", 'Defending the cross',
    'Ball-watching on the edge of the six-yard box — the runner you cannot see is the one who scores.',
    '盯着球看，站在小禁区边上——你看不见的那个插上的人，就是进球的那个。',
    '盯著球看，站在小禁區邊上——你看不見的那個插上的人，就是進球的那個。',
    'ゴールエリア際でボールばかり見る——見えていない走り込みが決める。',
    '골에어리어 근처에서 공만 본다 — 보이지 않는 침투가 골을 넣는다.',
    'Mirar solo el balón en el área pequeña: el que llega y no ves es el que marca.',
    'Regarder le ballon aux six mètres : celui que tu ne vois pas arriver est celui qui marque.',
    'Hanya menatap bola di tepi kotak kecil — pelari yang tak terlihat itulah yang mencetak gol.',
    'Hanya melihat bola di tepi kotak kecil; pelari yang tidak dilihat itulah yang menjaringkan.',
    'จ้องแต่บอลที่ขอบกรอบหกหลา คนที่วิ่งเข้ามาโดยไม่เห็นคือคนที่ยิงได้',
    'Chỉ nhìn bóng ở rìa vòng 5m50 — người bạn không thấy mới là người ghi bàn.')

add("soccer", 'Defending the throw-in',
    'The whole side drifts to the ball and nobody picks up the man peeling off behind them.',
    '全队都往球那边挪，没人管从身后绕出去的那个人。',
    '全隊都往球那邊挪，沒人管從身後繞出去的那個人。',
    '全員がボール側に流れ、背後へ抜ける選手を誰も掴まない。',
    '모두 공 쪽으로 쏠리고, 뒤로 빠지는 선수를 아무도 잡지 않는다.',
    'Todo el equipo se desplaza al balón y nadie coge al que se descuelga por detrás.',
    "Toute l'équipe glisse vers le ballon et personne ne prend celui qui décroche derrière.",
    'Seluruh tim bergeser ke arah bola dan tak ada yang menjaga pemain yang melepas di belakang.',
    'Seluruh pasukan menghala ke bola dan tiada siapa menjaga pemain yang melepasi di belakang.',
    'ทั้งทีมไหลไปทางบอล ไม่มีใครจับคนที่หลุดออกไปด้านหลัง',
    'Cả đội dạt về phía bóng và không ai bắt người tách ra phía sau.')

add("soccer", 'Rest defence',
    'Everyone joins the attack, so the counter starts 3v2 and nobody is even close enough to foul.',
    '所有人都压上去了，对方一反击就是 3 打 2，连个能上去犯规的人都够不着。',
    '所有人都壓上去了，對方一反擊就是 3 打 2，連個能上去犯規的人都夠不著。',
    '全員が攻撃に加わり、カウンターは3対2。ファウルで止める距離にすら誰もいない。',
    '모두 공격에 가담해 역습이 3대2로 시작되고, 파울로 끊을 거리에도 아무도 없다.',
    'Todos suben al ataque: el contra sale 3v2 y nadie llega ni a hacer falta.',
    "Tout le monde monte : le contre part à 3 contre 2 et personne n'est même assez près pour faire faute.",
    'Semua ikut menyerang, serangan balik jadi 3v2 dan tak ada yang cukup dekat untuk melanggar sekalipun.',
    'Semua naik menyerang; serangan balas menjadi 3v2 dan tiada siapa cukup dekat untuk membuat kesalahan.',
    'ทุกคนขึ้นเกมรุก สวนกลับกลายเป็น 3 ต่อ 2 และไม่มีใครใกล้พอจะฟาวล์ด้วยซ้ำ',
    'Tất cả dâng lên tấn công, pha phản công thành 3v2 và không ai đủ gần để phạm lỗi.')

add("soccer", 'Pressing trap: the touchline',
    'Pressing from outside in, which opens the switch the trap was built to stop.',
    '从外往里压，正好把这个陷阱本来要封死的那记转移让了出来。',
    '從外往裡壓，正好把這個陷阱本來要封死的那記轉移讓了出來。',
    '外から内へ寄せてしまい、この罠が止めるはずだったサイドチェンジを開ける。',
    '바깥에서 안으로 압박해, 트랩이 막으려던 전환을 열어준다.',
    'Presionar de fuera hacia dentro y abrir justo el cambio que la trampa quería cerrar.',
    "Presser de l'extérieur vers l'intérieur, ce qui ouvre le changement d'aile que le piège devait fermer.",
    'Menekan dari luar ke dalam, sehingga membuka umpan silang lapangan yang justru ingin dicegah.',
    'Menekan dari luar ke dalam, membuka hantaran silang padang yang perangkap itu cuba halang.',
    'กดจากนอกเข้าใน เปิดช่องเปลี่ยนข้างที่กับดักนี้ตั้งใจจะปิดพอดี',
    'Ép từ ngoài vào trong, mở ra chính đường chuyển cánh mà cái bẫy muốn chặn.')

add("soccer", 'Shooting from distance',
    'Leaning back over the ball — a shot hit that way clears the bar every time.',
    '打的时候身体后仰——这样打出来的球，每次都从横梁上飞过去。',
    '打的時候身體後仰——這樣打出來的球，每次都從橫梁上飛過去。',
    '上体が後ろに反る——その打ち方では毎回バーの上だ。',
    '몸이 뒤로 젖혀진다 — 그렇게 때리면 매번 크로스바를 넘는다.',
    'Echarse hacia atrás sobre el balón: así el disparo se va por encima siempre.',
    'Se pencher en arrière sur le ballon : frappée ainsi, elle passe au-dessus à chaque fois.',
    'Badan condong ke belakang saat menendang — tembakan seperti itu selalu melambung.',
    'Badan condong ke belakang ketika menendang; tendangan begitu sentiasa melambung.',
    'แอ่นตัวไปข้างหลังตอนยิง ยิงแบบนั้นข้ามคานทุกครั้ง',
    'Ngả người ra sau khi sút — cú sút kiểu đó lần nào cũng vọt xà.')

add("soccer", 'Second ball in the box',
    'Admiring the shot instead of following it — the rebound is gone before anyone moves.',
    '射完站着欣赏，没有跟进——补射的球早被人拿走了，你才刚要动。',
    '射完站著欣賞，沒有跟進——補射的球早被人拿走了，你才剛要動。',
    'シュートを見とれて詰めない——こぼれ球は動き出す前に消えている。',
    '슛을 감상하느라 따라 들어가지 않는다 — 세컨드 볼은 움직이기 전에 사라진다.',
    'Quedarse admirando el disparo en vez de seguirlo: el rechace se va antes de que nadie arranque.',
    'Admirer sa frappe au lieu de suivre : le renvoi est parti avant que quiconque bouge.',
    'Mengagumi tembakan alih-alih mengikutinya — bola muntah sudah hilang sebelum ada yang bergerak.',
    'Mengagumi tendangan dan tidak mengikutinya; bola muntah hilang sebelum sesiapa bergerak.',
    'มัวชื่นชมลูกยิงแทนที่จะตามเข้าไป บอลกระดอนหายไปก่อนใครจะขยับ',
    'Mải ngắm cú sút thay vì theo vào — bóng bật ra đã mất trước khi ai kịp di chuyển.')

add("soccer", 'Kick-off routine',
    'Improvising the one moment that was rehearsed, and handing the ball straight back.',
    '唯一排练过的那一下临场发挥，结果直接把球还给了对方。',
    '唯一排練過的那一下臨場發揮，結果直接把球還給了對方。',
    '唯一リハーサルした場面をその場で変え、そのままボールを返す。',
    '유일하게 연습한 장면을 즉흥으로 바꿔, 공을 그대로 돌려준다.',
    'Improvisar el único momento ensayado y devolver el balón de inmediato.',
    'Improviser le seul moment répété, et rendre le ballon aussitôt.',
    'Berimprovisasi pada satu momen yang sudah dilatih, lalu langsung menyerahkan bola kembali.',
    'Berimprovisasi pada satu-satunya saat yang dilatih, lalu terus memulangkan bola.',
